...
USER_QUERY = "Identify all the objects that are eligible to be picked up by the robot. If no objects are available then return an empty list."
...
def main():
    ...
    while True:
        ...
        print(f"Identified {len(target_ids)} target objects")
        ...
        for selected_target_id in target_ids:
            ...
            gp_tries = 0
#            valid_grasp_idxs = None
#            while not valid_grasp_idxs or gp_tries < 3:
            while gp_tries < 3:
                # -------------------------------------------------------------------------
                ...
                if not valid_grasp_idxs:
                    ...
                    gp_tries += 1
                    print(f"Retrying grasp prediction {gp_tries} times...")
                else:
                    break
